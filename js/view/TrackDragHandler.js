...
define( function( require ) {
  ...
  return inherit( SimpleDragHandler, TrackDragHandler, {
    ...
    trackDragged: function( event ) {
      ...
      if ( track.interactive == false ) { return; }
      ...
      // constrain translation in vertical direction (y) 
      track.position = new Vector2(location.x,track.position.y);
...
//		var distance = Math.abs(point.sourcePosition.x - otherPoint.position.x);
      ...
      if ( bestDistance !== null && bestDistance < MIN_DIST ) {
      ...
      if ( track.physical ) {
//        track.bumpAboveGround(); //this causes a bug when Y motion is constrained, track shifts vertically -Dinesh
      }
