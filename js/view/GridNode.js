...
//  var BackgroundNode = require( 'ROLLERCOASTER/view/BackgroundNode' );

  // strings
//  var zeroMetersString = require( 'string!ROLLERCOASTER/zeroMeters' );
   ...
   * @constructor
   */
  function GridNode( gridVisibleProperty, modelViewTransform, earthHeight ) {
  ...
  return inherit( Node, GridNode, {
    ...
    layout: function( offsetX, offsetY, width, height, layoutScale ) {
      ...
      var thinLines = [];
      var texts = [];
      var lineHeight = height / layoutScale - this.earthHeight;
      for ( var x = 0; x < 100; x++ ) {
      ...
      for ( var y = 0; y < 100; y++ ) {
        var originX = this.modelViewTransform.modelToViewX( -7 );
        ...
        if ( y % 1 === 0 ) {
          ...
          if ( y === 0 ) {
            var replacementText = new Text( "0 meters", {font: new PhetFont( 14 ), top: viewY, x: text.x} );
